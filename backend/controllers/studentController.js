...
const createStudent = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO students (first_name, middle_name, last_name, registration_no, symbol_no, gender, dob_bs, dob_ad, parent_name, enrollment_year, class_level, faculty, section, address, contact_no, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        first_name,
        middle_name,
        last_name,
...
const updateStudent = async (req, res) => {
  ...
  try {
    ...
    let updateQuery = `UPDATE students SET 
            ...
            parent_name=?, enrollment_year=?, class_level=?, faculty=?, section=?, 
            address=?, contact_no=?`;
    ...
    // Only update subjects if subject_ids is provided
    if (subject_ids) {
...
// ⭐ FIXED: Now includes enrolled subjects
